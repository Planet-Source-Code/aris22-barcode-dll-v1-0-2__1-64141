...
'http://grandzebu.net
Option Explicit
Sub GenPDF417(Obj As Object, BarText As String)
  
  Dim CodeBarre$, CodeErr%
  
  CodeBarre$ = PDF417_$(BarText, -1, 0, CodeErr%)
  'Returned and error
  If CodeErr% > 1 And CodeBarre$ = "" Then
    ...
    End Select
  End If
  Obj.FontName = "Code PDF417"
  Obj.FontSize = 20
  
  Obj.Width = (Obj.TextWidth(CodeBarre$) + 10) * Screen.TwipsPerPixelY
  Obj.Height = (Obj.TextHeight(CodeBarre$) + 10) * Screen.TwipsPerPixelX
  Obj.Cls
  Obj.CurrentY = 5
  Obj.Print CodeBarre$
End Sub

Private Function PDF417_$(Chaine$, Optional ByRef sécu%, Optional ByRef nbcol%, Optional ByRef CodeErr%)
  'V 1.2.0
  ...
  Dim ChaineMod$, Diviseur&, ChaineMult$, Nombre&, NbDigit%
  'Tables
  Dim ASCII$
  ...
  ASCII$ = ""
  Dim CoefRS$(8)
  ...
  For I% = 0 To IndexListe% - 1
    ...
    Case 900 'Texte
      ...
      For IndexListeT% = 0 To Liste%(0, I%) - 1
        GoSub QuelleTable
        ListeT%(0, IndexListeT%) = Table%
        ListeT%(1, IndexListeT%) = CodeASCII%
      Next
        ...
        Else
          ...
          If Not flag Then 'On re-teste flag qui a peut-être changé ci-dessus ! donc ELSE pas possible / We test again flag which is perhaps changed ! Impossible tio use ELSE statement
            ...
            If J% = Liste%(0, I%) - 1 Then
              NewTable% = ListeT%(0, J%)
            Else
              NewTable% = ListeT%(0, J%) And ListeT%(0, J%)
            End If
  ...
  For I% = 0 To Len(ChaineMC$) / 3 / nbcol% - 1
    ...
    End Select
    PDF417_$ = PDF417_$ & "+*" 'Commencer par car. de start et séparateur / Start with a start char. and a separator
    For J% = 0 To Len(Dummy$) / 3 - 1
      PDF417_$ = PDF417_$ & Mid$(CodageMC$(I% Mod 3), Mid$(Dummy$, J% * 3 + 1, 3) * 3 + 1, 3) & "*"
    Next
    PDF417_$ = PDF417_$ & "-" & Chr$(13) & Chr$(10) 'Ajouter car. de stop et CRLF / Add a stop char. and a CRLF
  Next
  Exit Function
Regroupe:
...
QuelleTable:
  ...
  Case 13 'CR
    ...
    CodeASCII% = 11
  Case Else
    Table% = Mid$(ASCII$, CodeASCII% * 4 - 127, 2)
    CodeASCII% = Mid$(ASCII$, CodeASCII% * 4 - 125, 2)
  End Select
Return
Modulo:
  ...
  ChaineMult$ = ""
  Do
    Nombre& = Val(Left$(ChaineMod$, 9))
    If Len(ChaineMod$) > 9 Then ChaineMod$ = Mid$(ChaineMod$, 10) Else ChaineMod$ = ""
    NbDigit% = Len(ChaineMod$)
    ChaineMod$ = Format(Nombre& Mod Diviseur&, "general number") & ChaineMod$
    ChaineMult$ = ChaineMult$ & Format(Nombre& \ Diviseur&, "general number")
    If Len(ChaineMod$) <= 9 Then
      If Val(ChaineMod$) < Diviseur& Then
        ChaineMult$ = ChaineMult$ & String$(NbDigit%, "0")
        Exit Do
      End If
    End If
  Loop
  Diviseur& = Val(ChaineMod$)
Return
End Function
